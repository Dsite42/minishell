...
/*   Updated: 2023/07/05 11:24:16 by cgodecke         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	dup_read(int *fd_dup, t_redir *head_redir, t_piping *piping_data)
{
	int	fd_infile;
...
static void	dup_read_heredoc(t_piping *piping_data, int *fd_dup)
{
	...
	while (head_redir != NULL)
	{
		if (head_redir->type == WORD_OP_READ)
			dup_read(fd_dup, head_redir, piping_data);
		else if (head_redir->type == WORD_OP_HEREDOC
			&& head_redir->last_heredoc == 1)
		{
			fd_infile = head_redir->pipefd_heredoc[0];
			if (fd_infile == -1)
				execution_error(1, "fd_error heredoc", 1, errno);
			fd_dup[0] = dup2(fd_infile, STDIN_FILENO);
			if (fd_dup[0] == -1)
				execution_error(1, "dup_error heredoc", 1, errno);
			close(fd_infile);
		}
...
void	input_redirection(t_piping *piping_data, int *fd_dup)
{
	...
	else if (piping_data->prev_read != STDIN_FILENO)
	{
		...
		if (fd_dup[0] == -1)
			execution_error(1, "dup_error prev_read", 1, errno);
	}
}
